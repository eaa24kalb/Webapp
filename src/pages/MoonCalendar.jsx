...
export default function MoonCalendar() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      try {
        ...
        let idx = w.findIndex((week) =>
          week.some((d) => d && d.date === todayIso)
        );
        if (idx === -1) idx = 0;
        setWeekIndex(idx);
  ...
  const time = (iso) =>
    iso ? new Date(iso).toLocaleTimeString([], { timeStyle: "short" }) : "—";
    ...
    <div className={styles.page}>
      <section className={styles.hero}>
        ...
        <div className={styles.phaseSub}>
          {selected?.illumination != null
            ? `${selected.illumination}% illuminated`
            : "—"}
        </div>
        ...
        <div className={styles.moonWrap}>
          <div className={styles.moonImg} aria-hidden />
        </div>
        ...
        <div className={styles.timesRow}>
          <div className={styles.timeCol}>
            ...
            <div className={styles.timeText}>{time(selected?.rise)}</div>
          </div>
          <div className={styles.timeCol}>
            <span className={styles.timeIcon}>🌙⬇️</span>
            <div className={styles.timeText}>{time(selected?.set)}</div>
          </div>
          <div className={styles.timeCol}>
            <span className={styles.timeIcon}>☀️⬆️</span>
            <div className={styles.timeText}>{time(selected?.sunrise)}</div>
          </div>
          <div className={styles.timeCol}>
            <span className={styles.timeIcon}>☀️⬇️</span>
            <div className={styles.timeText}>{time(selected?.sunset)}</div>
          </div>
        </div>
        ...
        <div className={styles.infoGrid}>
          ...
          <div className={styles.infoRow}>
            <span>Moon Age:</span>
            <strong>
              {selected?.age != null ? `${selected.age} days` : "—"}
            </strong>
          </div>
          <div className={styles.infoRow}>
            <span>Moon Zodiac:</span>
            <strong>{selected?.sign || "—"}</strong>
          </div>
          <div className={styles.infoRow}>
            <span>Altitude:</span>
            <strong>
              {selected?.altitude != null ? `${selected.altitude}°` : "—"}
            </strong>
          </div>
          <div className={styles.infoRow}>
            <span>Azimuth:</span>
            <strong>
              {selected?.azimuth != null ? `${selected.azimuth}°` : "—"}
            </strong>
          </div>
        </div>
        ...
        <div className={styles.ritualsWrap}>
          ...
          <div className={styles.ritualGrid}>
            {(() => {
              const key = (selected?.phaseId || "unknown")
                .toLowerCase()
                .replace(/\s+/g, "_");
              const map = {
                new_moon: [
                  "New Moon — Set intentions",
                  "Moon water — charge jar overnight",
                ],
                waxing_crescent: ["Plant seeds — small steps", "Carry citrine"],
                first_quarter: [
                  "Take decisive action",
                  "Short movement ritual",
                ],
